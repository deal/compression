...
In addition, if a response contains an ETag, `@deal/compression` will cache the compressed
...
`@deal/compression` serve static files in [our benchmark](./benchmark) 3x faster than
...
```js
var compression = require('@deal/compression')
```

### compression([options])

Returns the @deal/compression middleware using the given `options`. The middleware
...
Note that `@deal/compression` options are backward-compatible with `compression`, but
...
of options in `@deal/compression`, you will get a deprecation warning.
...
be cached. Note that `@deal/compression` uses ETags to ensure that a cache entry is appropriate
...
By default, `@deal/compression` caches any response that has an `ETag` header associated with
